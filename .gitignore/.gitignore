...
define('BOT_TOKEN', '715631213:AAFdkGHUcP-44p9smSZrvh4_CgcUnidwMgQ');
...
function processMessage($message) {
  ...
  if (isset($message['text'])) {
    ...
    if (strpos($text, "/start") === 0) {
      apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" => "مرحبا , لعبة ox😄\nمتوفره على منصة التلي كرام ⚡️\nيمكنك لعبها مع اصدقائك 🎊''", 'reply_markup' => array(
        "inline_keyboard"=>array(
          array(array("text"=>"لعب XO","switch_inline_query"=>md5(date("YMDms"))),array("text"=>"✅","callback_data"=>"m"))
          )
    )));
    } 
  }
...
array(array("text"=>"تابع 💡
","url"=>"https://telegram.me/Touch_t")),
)
    )
  ))));
  exit;
...
define('WEBHOOK_URL', 'https://xobot-steph2016.c9users.io/ox.php');
